...
이 문서는 회원가입 요청이 애플리케이션과 PostgreSQL을 거쳐 응답으로 반환되는 전체 흐름을 설명한다. 세부 구현을 대신하지 않고, 각 단계의 책임과 관련 코드를 찾아가기 위한 지도를 제공한다.
...
- 입력 검증을 통과해야 Registration Endpoint가 실행된다.
...
| 조건                                                | 처리 결과                                                   | 데이터베이스 영향                          |
| --------------------------------------------------- | ----------------------------------------------------------- | ------------------------------------------ |
| 요청 구조, 필수값, email 또는 공백 문자열 검증 실패 | Endpoint를 실행하지 않고`422` 오류를 반환한다.            | 사용자를 저장하지 않는다.                  |
| PostgreSQL의 username 또는 email 고유성 제약 위반   | 트랜잭션을 rollback하고 해당 필드의`409` 오류로 변환한다. | 기존 사용자와 사용자 수가 변경되지 않는다. |
| 그 밖의 데이터베이스 쓰기 실패                      | 트랜잭션을 rollback하고 오류를 전파하여`500`을 반환한다.  | 실패한 사용자를 저장하지 않는다.           |
